...
DB_CONNECTION = (
    "DATABASE=DEPLOY;"
    f"HOSTNAME={os.environ['DB_DEPLOY_HOSTNAME']};"
    f"PORT={os.environ['DB_DEPLOY_PORT']};"
    f"PROTOCOL=TCPIP;"
    f"UID={os.environ['DB_DEPLOY_UID']};"
    f"PWD={os.environ['DB_DEPLOY_PASSWORD']};"
)

try:
    conn = ibm_db.connect(DB_CONNECTION, "", "")
    ibm_db.exec_immediate(conn, "SET CURRENT SCHEMA DEPLOYZ")
    print("✓ Connection successful!")
except Exception as e:
    print("â�Œ Connection error:", e)
    exit(1)
...
def run_query(sql, params=None):
    try:
        ...
        print("âš ï¸� Error executing query:", e)
        return []
